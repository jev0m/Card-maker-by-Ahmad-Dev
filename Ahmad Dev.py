...
def clear():
      if os.name=='nt':
          os.system('cls && title [NEW] Card Maker - FULL KEY GENERAT0R BY Jev0m ')
      else :
          os.system('clear')
#------------#
def strounga(x,y):
...
logo=choice(["""{w}
▒▒▄▀▀▀▀▀▄▒▒▒▒▒▄▄▄▄▄▒▒▒
▒▐░▄░░░▄░▌▒▒▄█▄█▄█▄█▄▒  {r}({y}Instagram{r}){rr}~~~~~~~~~{r}({y}@Jev0m.tn{r}){w} 
▒▐░▀▀░▀▀░▌▒▒▒▒▒░░░▒▒▒▒  {r}({y}Telegram{r}){rr}~~~~~~~~~~{r}({y}@Div0m{r}){w}
▒▒▀▄░═░▄▀▒▒▒▒▒▒░░░▒▒▒▒
▒▒▐░▀▄▀░▌▒▒▒▒▒▒░░░▒▒▒▒
...
{rr}*{r} Instagam     {rr}:{y}@X0.id
{rr}*{r} Telegram     {rr}:{y}@Div0m
...
{rr}\\_{r}Instagam{rr}>{y}@x0.id
{rr}\\_{r}Telegram{rr}>{y}@Div0m
...
class ccSama(object):
  ...
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"""%(blid,lasfar,lazra9,lasfar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar))
    def gen1(self ,Sa7by):#Sa7by Kol Chy Wadha7 W jawna Bahy Brcha xDD...
      ...
      with open('Card maker-results/ccgenerated.txt','a') as escnoor:
        escnoor.write('\n~~ Bin:%s | %s cc ~~ (By Ahmad Dev) \n'%(binn3r,mouchi))
      for ii in range(mouchi):
        ...
        print(la7mar+sama)
        with open('Card make-results/ccgenerated.txt','a') as escnoor:
          escnoor.write(sama+'\n')
      print("%s+%s--------------------------%s+"%(la7mar,lasfar,la7mar))
      print(" %s[%s%s Saved In 'Card maker-results/ccgenerated.txt'%s]%s"%(la7mar,lasfar,mouchi,la7mar,labyadh))
...
class ghrab(object):
    def __init__(self):
        ...
        for ip in ip_range:
            xXx=xXx+1
            with open('Card make-results/Ipgenerated.txt', 'a') as xX:
                xX.write(str(ip) + ':' + str(PRoxYPort) + '\n')
        print("%s[%s%s Ip Saved In 'Card make-results/Ipgenerated.txt'%s]%s"%(la7mar,lasfar,xXx,la7mar,labyadh))
        so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
        if so.upper() in ['Y','YES','AY']:
            z4ck()
        else:
  ...
  def __init__(self):
      ...
      if baka<=xnumx:
          ...
          for i in range(ma):
             ...
             with open('Card make-results/%s.txt'%(conf['name']),'a') as L:
               L.write('%s\n'%(self.x1x(conf['format'])))
          print(" %s[%s%s Key Saved In 'Card make-results/%s.txt' %s]%s"%(la7mar,lasfar,ma,conf['name'],la7mar,labyadh))
          so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
          if so.upper() in ['Y','YES','AY']:
              z4ck()
          else:
...
#1,2,3 Go!
  ...
  def x1x(self,x):
   ...
   return x444
  def phnnm(self):
    if not os.path.isdir('Card make-results/phonz'):
     os.mkdir('Card make-results/phonz')
    print("""
        Helper:
    ...
    for i in range(Hwmch):
      ...
      ousm='+'+self.x1x(bakas)
      print(ousm)
      with open('Card make-results/phonz/C0de_%s.txt'%(str(intcd)),'a') as L:
        L.write('%s\n'%ousm)
    print(" %s[%s%s number phone Saved In 'Card make-results/phonz/C0de_%s.txt'%s]%s"%(la7mar,lasfar,Hwmch,str(intcd),la7mar,labyadh))
    so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
    if so.upper() in ['Y','YES','AY']:
           z4ck()
    else:
...
#fbtokengen
  def genetok(self):
   ...
   for az in range(int(zz)):
    ...
    print(escnoor)
    with open('Card make-results/tokens.txt','a') as ttt:
      ttt.write(str(escnoor)+'\n')
   print(" %s[%s%s Token Saved In 'Card make-results/tokens.txt'%s]%s"%(la7mar,lasfar,zz,la7mar,labyadh))
   so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
   if so.upper() in ['Y','YES','AY']:
            z4ck()
   else:
...
#Key creator 
  ...
  def seemydick(self):
   print("""{r}
     ({y}Name/Nickname{r}){rr}~~{r}({y}Ahmad{rr}/{y}jev0m{r})
...
#------------#    
try:
  if not os.path.isdir('Card make-results'):
    os.mkdir('Card make-results')
  if not os.path.isfile('config.ini')or len(open('config.ini','r').read())<1:
      ...
      open('config.ini','w').write('#Card make\n')
